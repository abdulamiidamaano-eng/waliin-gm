...
const JWT_SECRET =
  process.env.JWT_SECRET || "waliin-gm-change-this-secret";
...
async function initDatabase() {
  ...
  try {
    ...
    /*
      IMPORTANT:
      Yoo users table duraan jiraatee status hin qabne,
      CREATE TABLE IF NOT EXISTS hin dabalu.
      Kanaaf ALTER addaatti raawwatama.
    */
    ...
    try {
      ...
      if (!validUsername(username)) {
        return res.status(400).json({
          error:
            "Username 3-50 characters qabaachuu qaba."
        });
      }
    ...
    socket.on(
      ...
      () => {
        try {
          ...
          if (
            !club.seatRequests.includes(
              username
            )
          ) {
            club.seatRequests.push(
              username
            );
          }
